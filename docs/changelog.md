...
- **`rollup.archive_raw` now defaults to `true`**: rolled-up raw episodic rows
  ...
  in `~/.cairn/config.json` to restore deletion. Disk note: archived raws
...
- Browse/phrase search paths (`get_by_type`, `get_by_session`, `get_recent`,
  ...
  query pipeline. Explicit fetch by id (`cairn_memory action=get`) still
...
### Added
- `cairn_memory action=get`: fetch full memory content by id or unique prefix.
- Daemon-side maintenance scheduler (`cairn.scheduler`): rollup, link, gc,
...
- **Free-tier conversion funnel**: Milestone CTAs surfaced at 500 / 1,000 / 1,500 / 2,000
  captured memories with upgrade links to `tracqi.com/pro`. License-aware: Pro users see
...
- **Tier-aware memory cap**: Free tier soft cap at 2,000 (retrieval quality degrades) and
  hard write cap at 5,000 with grandfathering for existing larger DBs. `CAIRN_MAX_NODES`
  env honored for Pro only. `cairn status` exposes `tier` / `soft_cap` / `hard_cap` for
  Free users.
- **`CAIRN_AUTO_RELATE` env opt-out**: Parity with `CAIRN_ENTITY_EXTRACTION` — set to
...
- **MCP Pro tool discovery**: now correctly references the `cairn_platform` namespace.
