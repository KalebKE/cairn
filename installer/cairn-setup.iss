; Cairn - Windows Installer (Inno Setup 6)
;
; Bundles Python 3.12 embeddable, installs cairn[server] via pip,
; and auto-configures Claude Desktop's MCP config.
;
; Build: iscc cairn-setup.iss (or via GitHub Actions)
...
DefaultDirName={localappdata}\Cairn
DefaultGroupName={#MyAppName}
DisableProgramGroupPage=yes
PrivilegesRequired=lowest
OutputDir=dist
OutputBaseFilename=cairn-setup
Compression=lzma2
...
Name: "{group}\Repair Cairn Config"; Filename: "{app}\repair-config.bat"; \
  Comment: "Fix Cairn connection to Claude Desktop"

[UninstallRun]
; Remove Cairn from Claude Desktop config before uninstalling
Filename: "{app}\python\python.exe"; \
...
[Messages]
WelcomeLabel2=This will install {#MyAppName} for Claude Desktop.%n%nCAIRN gives Claude persistent memory across conversations.%n%nRequirements:%n- Claude Desktop must be installed%n- Internet connection (for downloading the embedding model on first use)
FinishedLabel=Setup has installed {#MyAppName} for Claude Desktop.%n%nRestart Claude Desktop to start using Cairn. Say "hello" to Claude and Cairn's memory tools will be available.

[Code]
function ClaudeConfigHasCairn(): Boolean;
var
  ConfigPath: String;
  Content: AnsiString;
begin
...
begin
  ...
  begin
    if not ClaudeConfigHasCairn() then
    begin
      MsgBox(
        'Cairn was installed but could not configure Claude Desktop automatically.' + #13#10 + #13#10 +
        'To fix this:' + #13#10 +
        '1. Open Start Menu and find "Repair Cairn Config"' + #13#10 +
