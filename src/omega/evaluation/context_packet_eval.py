...
"""Context packet quality evaluation for OMEGA.
...
from omega.evaluation.retrieval_eval import Probe, generate_keyword_probe, sample_memories
from omega.server.context_handlers import build_context_packet, _estimate_tokens
    ...
    for probe in probe_results:
        ...
        for target_id in probe.get("memory_ids_used") or []:
            ...
            if target is None or not target_emb:
                continue
            try:
                from omega.sqlite_store._types import _cosine_similarity
...
def format_packet_diagnosis(diagnosis: Dict[str, Any]) -> str:
    ...
    lines = [
        "# OMEGA Context Packet Diagnosis",
        "",
...
def format_packet_report(report: PacketEvalReport) -> str:
    ...
    lines = [
        "# OMEGA Context Packet Evaluation Report",
        "",
