...
via the configured LLM (``cairn.llm.llm_complete`` — routed through the local
...
logger = logging.getLogger("cairn.rollup")
...
def _archive_raw_config() -> bool:
    """Read rollup.archive_raw from ~/.cairn/config.json (default True:
    ...
    try:
        cfg_path = Path(os.environ.get("CAIRN_HOME", os.path.expanduser("~/.cairn"))) / "config.json"
