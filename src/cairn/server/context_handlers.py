...
context_assemble returns Cairn's active context as a list of structured
sections plus a pre-rendered markdown blob. The six section ids map
honestly onto Cairn's event_type model -- no invented categories.
...
from cairn.server.responses import mcp_error, mcp_response

logger = logging.getLogger("cairn.server.context_handlers")
...
def _packet_query_text(task: str, files: List[str]) -> str:
    ...
    from cairn.server.hook_server import _natural_tokens
    parts: List[str] = []
    if task:
        parts.append(task)
...
async def handle_context_assemble(arguments: dict) -> dict:
    ...
    try:
        from cairn.bridge._core import _get_store
        db = _get_store()
    except Exception as exc:
