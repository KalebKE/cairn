...
Exposes Cairn's "active context digest" as structured sections that any
...
Coexists with cairn_welcome (which keeps its rich Claude-Code-shaped
Dict return). Same SQLite source, different face.

The section list is an honest mapping to Cairn's actual event_type model:
no invented "identity" or "hard_rules" sections -- those belong to the
caller's harness layer, not Cairn.
"""
...
CONTEXT_TOOL_SCHEMAS = [
    {
        ...
        "description": (
            "Assemble Cairn's active context as structured sections plus a "
            "pre-rendered markdown blob. Harness-agnostic; clients splice into "
            "their own system prompt. For Claude Code, cairn_welcome remains the "
