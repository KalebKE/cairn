"""Lean Cairn hook server — fast in-daemon memory surfacing over a Unix socket.
...
for the client (``cairn/hooks/fast_hook.py``) to print into the model's context.

Design goals (driven by the historical failure mode — see ~/.cairn/LESSONS.md):
...
* **Token-lean by construction.**  Budgets (result count, relevance floor,
  ...
  conservative defaults and read from ``~/.cairn/config.json`` under ``surface``.
...
logger = logging.getLogger("cairn.hook_server")


def _cairn_home() -> Path:
    return Path(os.environ.get("CAIRN_HOME", os.path.expanduser("~/.cairn")))


# Must resolve to the same path the client uses (fast_hook.py: ~/.cairn/hook.sock).
SOCK_PATH: Path = _cairn_home() / "hook.sock"

# Conservative defaults; override any subset via ~/.cairn/config.json:
...
def _do_surface(payload: Dict[str, Any]) -> str:
    ...
    from cairn.server import context_handlers as _ctx
    ...
    def _emit(pkt) -> None:
        ...
        try:
            from cairn.telemetry import track_context_packet
...
# otherwise depends entirely on the agent remembering to call cairn_store.
...
async def start_hook_server() -> "asyncio.AbstractServer | None":
    ...
    try:
        ...
        _bound_inode = None
    logger.info("Cairn hook server listening at %s", SOCK_PATH)
    return _server
