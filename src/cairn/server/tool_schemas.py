...
cairn_briefing and cairn_habits remain as backward-compat aliases in handlers.
cairn_lessons removed — cross-session lessons auto-surface via hooks on file edits.

Condensed Mode (CodeMode-inspired):
  When CAIRN_CONDENSED=1, only 5 tools are exposed: 3 standalone essentials
  (cairn_welcome, cairn_protocol, cairn_store) + 2 meta-tools (cairn_tools,
  cairn_call). All other tools are accessible via cairn_call(tool=..., args=...).
...
STANDALONE_TOOLS = ["cairn_welcome", "cairn_protocol", "cairn_store"]
...
def get_condensed_schemas(all_schemas: list[dict]) -> list[dict]:
    ...
    All other tools are accessible via cairn_call/cairn_tools meta-tools.
    """
