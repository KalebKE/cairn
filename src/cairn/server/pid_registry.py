"""PID registry for local Cairn server processes.
...
def list_registered_pids(clean_stale: bool = True) -> list[dict[str, Any]]:
    """Return live registered Cairn server processes."""
...
def format_lock_diagnostic() -> str:
    ...
    if not entries:
        return "No registered Cairn server processes found"
    parts = []
    ...
    return "Registered Cairn processes: " + "; ".join(parts) + suffix
