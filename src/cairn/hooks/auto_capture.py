#!/usr/bin/env python3
"""Cairn UserPromptSubmit hook — Auto-capture decisions and lessons from user prompts.

Fires on every user prompt. Detects decision and lesson patterns and stores them
as 'decision' or 'lesson_learned' event type in Cairn memory. Uses conservative
...
def _echo_capture(result: str, event_type: str, prompt: str):
    ...
    - New capture → [Cairn] Captured: decision about X
    - Evolution   → [Cairn] Memory evolved: added insight to existing memory
    ...
    if "Memory Evolved" in result:
        ...
        print(f"[Cairn] Memory evolved: {event_type} updated (evolution #{evo_num}) — {summary}")
    elif "Memory Captured" in result:
        print(f"[Cairn] Captured: {event_type} — {summary}")
