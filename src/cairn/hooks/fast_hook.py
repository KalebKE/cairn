#!/usr/bin/env python3
"""Cairn fast hook client — routes to daemon via UDS, falls back to direct.

This is a thin client that connects to the hook server running inside the
MCP process. It avoids importing any Cairn modules on the fast path, keeping
...
def _detect_client() -> str:
    ...
    The env var CAIRN_CLIENT is set by `cairn setup --client <name>`.
    """
    client = os.environ.get("CAIRN_CLIENT", "")
    if client:
        return client
...
def _fallback(hook_name, payload):
    ...
    if not script_name:
        if hook_name in _BLOCKING_HOOKS:
            print(f"Cairn: blocking hook '{hook_name}' has no fallback script — daemon-only", file=sys.stderr)
        return
    ...
    try:
        ...
        print(f"Cairn hook fallback error ({hook_name}): {e}", file=sys.stderr)
