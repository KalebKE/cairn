...
from cairn.exceptions import EmbeddingError, StorageError, ValidationError
...
class MaintenanceMixin:
    ...
    def reembed_all(self, batch_size: int = 32) -> Dict[str, int]:
        ...
        """
        from cairn.embedding import generate_embeddings_batch, get_active_backend
    ...
    def backfill_embeddings(self, batch_size: int = 50) -> dict:
        ...
        from cairn.embedding import generate_embedding, get_active_backend, is_embedding_degraded
    ...
    def get_agency_tax(self) -> Dict[str, Any]:
        ...
        Cairn targets < 500ms for writes, < 200ms for reads.
        """
        result: Dict[str, Any] = {}
    ...
    def check_memory_health(
        ...
        if rss_mb > critical_mb:
            ...
            try:
                self._invalidate_query_cache()
                from cairn.embedding import _EMBEDDING_CACHE
        ...
        if count > max_nodes:
            ...
            recommendations.append("Run cairn consolidate to deduplicate and prune")
        ...
        if backlog["backlog_critical"]:
            ...
            recommendations.append("Run cairn_maintain(action='consolidate') to clear backlog")
    ...
    def export_to_file(self, filepath: Path) -> Dict[str, Any]:
        ...
        # Encrypt export if CAIRN_ENCRYPT is enabled
        from cairn.crypto import is_enabled as crypto_enabled, encrypt as crypto_encrypt
        if crypto_enabled():
    ...
    def import_from_file(self, filepath: Path, clear_existing: bool = True) -> Dict[str, Any]:
        ...
        if raw_content.startswith("ENC:"):
            from cairn.crypto import decrypt as crypto_decrypt
