"""
Cairn Migration: JSON graphs + JSONL sidecar → SQLite + sqlite-vec.
...
Usage:
    cairn migrate-db           # Interactive migration
    cairn migrate-db --force   # Overwrite existing database
"""
...
logger = logging.getLogger("cairn.migrate")

CAIRN_DIR = Path(os.environ.get("CAIRN_HOME", str(Path.home() / ".cairn")))
...
def auto_migrate_if_needed() -> bool:
    ...
    has_jsonl = (CAIRN_DIR / "store.jsonl").exists()

    if not has_graphs and not has_jsonl:
        return False  # Fresh install, no migration needed

    print("Cairn: Auto-migrating to SQLite backend...")
    try:
        report = migrate(force=False)
        if report.get("nodes_migrated", 0) > 0:
            print(f"Cairn: Migrated {report['nodes_migrated']} memories to SQLite.")
            return True
        return False
    except Exception as e:
        logger.error(f"Auto-migration failed: {e}", exc_info=True)
        print(f"Cairn: Auto-migration failed: {e}")
        print("  Run 'cairn migrate-db --force' to retry manually.")
        return False
