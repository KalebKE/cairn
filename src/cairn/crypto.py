"""
Cairn Crypto -- Optional encryption at rest for memory stores.
...
machine-specific secret stored at ~/.cairn/.key.

Enabled by default since v1.1.
Disable: CAIRN_ENCRYPT=0
...
logger = logging.getLogger("cairn.crypto")
...
def _cairn_home() -> Path:
    """Resolve CAIRN_HOME lazily so tests can override via env var."""
    return Path(os.environ.get("CAIRN_HOME", str(Path.home() / ".cairn")))


def _key_path() -> Path:
    """Resolve key file path lazily."""
    return _cairn_home() / ".key"


def is_enabled() -> bool:
    """Check if encryption at rest is enabled (on by default since v1.1).

    Set CAIRN_ENCRYPT=0 to explicitly disable.
    """
    val = os.environ.get("CAIRN_ENCRYPT", "").strip().lower()
...
def _get_or_create_key() -> bytes:
    ...
    cairn_home = _cairn_home()
    cairn_home.mkdir(parents=True, exist_ok=True, mode=0o700)
...
def _get_fernet():
    ...
    try:
        ...
        logger.warning("CAIRN_ENCRYPT=1 but 'cryptography' package not installed. Run: pip install cryptography")
        return None
