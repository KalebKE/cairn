"""
Cairn Cross-Encoder Reranker — ONNX-based passage reranking for search quality.
...
- Env-var disable: CAIRN_CROSS_ENCODER=0
- Precision selection: CAIRN_RERANKER_PRECISION=fp32|int8 (default: int8)
  fp32: full precision, ~2.3 GB on disk, ~2.5 GB RSS
  int8: quantized, ~571 MB on disk, ~650 MB RSS
- Auto-download on first use: CAIRN_RERANKER_AUTODOWNLOAD=1 (default).
  Set to 0 to require manual download via `python -m cairn --download-model`.

Uses cross-encoder/ms-marco-MiniLM-L-6-v2 via ONNX Runtime.
Mirrors the loading patterns from cairn.embedding.
"""
...
logger = logging.getLogger("cairn.reranker")
...
# Override model with CAIRN_RERANKER_MODEL, precision with CAIRN_RERANKER_PRECISION.
_AVAILABLE_MODELS = {
    "bge-reranker-v2-m3": {
        ...
        # Set CAIRN_RERANKER_PRECISION=fp32 to opt back in.
        "default_precision": "int8",
        "precisions": {
            "fp32": {
                "dir": "~/.cache/cairn/models/bge-reranker-v2-m3-onnx",
                "files": [
            ...
            "int8": {
                "dir": "~/.cache/cairn/models/bge-reranker-v2-m3-onnx-int8",
                "files": [
    ...
    "ms-marco-MiniLM-L-6-v2": {
        ...
        "dir": "~/.cache/cairn/models/ms-marco-MiniLM-L-6-v2-onnx",
        "files": [
            ("onnx/model.onnx", "model.onnx"),
            ("tokenizer.json", "tokenizer.json"),
            ("config.json", "config.json"),
...
def _resolve_model_config(model_name: str, precision: str | None = None) -> tuple[str, str, list[tuple[str, str]]]:
    ...
    if "precisions" in model:
        if precision is None:
            precision = os.environ.get("CAIRN_RERANKER_PRECISION", model["default_precision"])
...
def cross_encoder_score(
    ...
    if os.environ.get("CAIRN_CROSS_ENCODER") == "0":
        logger.debug("Cross-encoder disabled (CAIRN_CROSS_ENCODER=0)")
        return None
...
def download_model(
    ...
    Args:
        ...
        precision: "fp32" (~2.3 GB) or "int8" (~571 MB). Defaults to
            CAIRN_RERANKER_PRECISION env var, then "fp32".
...
def _get_model_dir() -> str | None:
    ...
    env_dir = os.environ.get("CAIRN_CROSS_ENCODER_DIR")
    if env_dir:
...
def _get_reranker_model():
    ...
    if os.environ.get("CAIRN_CROSS_ENCODER") == "0":
        logger.info("Cross-encoder disabled (CAIRN_CROSS_ENCODER=0)")
        return None
    ...
    if model_dir is None:
        if os.environ.get("CAIRN_RERANKER_AUTODOWNLOAD", "1") == "0":
            logger.warning(
                "Cross-encoder model not found and CAIRN_RERANKER_AUTODOWNLOAD=0. "
        ...
        if downloaded is None:
            logger.warning(
                ...
                "Run `python -m cairn --download-model` manually to retry."
            )
            return None
        model_dir = _get_model_dir()
        if model_dir is None:
