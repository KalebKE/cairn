...
<#
.SYNOPSIS
    Cairn Pro installer for Windows + Claude Desktop.
...
.DESCRIPTION
    ...
    3. Creates an isolated venv at %LOCALAPPDATA%\Cairn\venv via uv
    4. Installs cairn (core) from PyPI via uv
    5. Finds and installs the Pro wheel from Downloads/Desktop
    6. Runs cairn setup (creates ~/.cairn/, downloads ONNX model)
...
$CairnRoot     = Join-Path $env:LOCALAPPDATA "Cairn"
$VenvDir       = Join-Path $CairnRoot "venv"
$VenvPython    = Join-Path $VenvDir "Scripts\python.exe"
$VenvPip       = Join-Path $VenvDir "Scripts\pip.exe"
$CairnHome     = Join-Path $env:USERPROFILE ".cairn"
...
Write-Host "  ║       Cairn Pro  —  Windows Setup     ║" -ForegroundColor DarkYellow
Write-Host "  ╚══════════════════════════════════════╝" -ForegroundColor DarkYellow
Write-Host ""
...
if (-not (Test-Path $CairnRoot)) {
    New-Item -ItemType Directory -Path $CairnRoot -Force | Out-Null
}
...
function Find-ProWheel {
    ...
    foreach ($dir in $searchDirs) {
        if (Test-Path $dir) {
            $wheel = Get-ChildItem -Path $dir -Filter "cairn_memory_pro-*.whl" -ErrorAction SilentlyContinue |
...
if ($UseUv) {
    ...
    # Install core
    try {
        & $UvExe pip install cairn --python $VenvPython --quiet
        Write-OK "cairn installed (via uv)"
    } catch {
        Exit-WithError "Failed to install cairn: $_"
    }
    ...
    if (-not $proWheel) {
        ...
        Write-Host "  Please download it from https://admin.tracqi.com/pro/dashboard" -ForegroundColor Gray
        Write-Host "  then press any key to continue..." -ForegroundColor Yellow
        $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
        $proWheel = Find-ProWheel
    }
    if (-not $proWheel) {
        Exit-WithError "Could not find cairn_memory_pro-*.whl. Download it from the Pro dashboard and try again."
    }
    ...
    # Install core
    try {
        & $VenvPip install cairn --quiet
        Write-OK "cairn installed (via pip)"
    } catch {
        Exit-WithError "Failed to install cairn: $_"
    }
...
# ═══════════════════════════════════════════════════════════════════════
# STEP 4: Run cairn setup
# ═══════════════════════════════════════════════════════════════════════

Write-Step 4 "Running cairn setup"

try {
    & $VenvPython -m cairn setup --client claude-desktop 2>&1 | ForEach-Object { Write-Host "  $_" }
    Write-OK "cairn setup complete"
} catch {
    Write-WARN "cairn setup had warnings (this may be OK): $_"
}
...
if ([string]::IsNullOrWhiteSpace($licenseKey)) {
    Write-WARN "No license key entered. You can activate later with: cairn activate <key>"
} else {
    try {
        & $VenvPython -m cairn activate $licenseKey 2>&1 | ForEach-Object { Write-Host "  $_" }
        Write-OK "License activated"
    } catch {
        Write-WARN "License activation issue: $_"
        Write-Host "  You can try again later with: & '$VenvPython' -m cairn activate $licenseKey" -ForegroundColor Gray
    }
}
...
Start-Process "https://admin.tracqi.com/pro/dashboard"
...
Write-Host "    Cairn:    $CairnHome" -ForegroundColor Gray
...
Write-Host "  Then ask Claude: 'What Cairn tools are available?'" -ForegroundColor Yellow
Write-Host ""
