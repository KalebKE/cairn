...
End-to-end acceptance tests for the Cairn memory system.
...
class TestUATMemoryLifecycle:
    ...
    async def test_store_returns_node_id(self):
        ...
        result = await HANDLERS["cairn_store"]({
            "content": "Always use pytest-asyncio for async test fixtures in Cairn project",
    ...
    async def test_store_and_query_roundtrip(self):
        ...
        result = await HANDLERS["cairn_query"]({"query": "SQLite WAL concurrent read"})
        assert not _is_error(result)
        text = _text(result)
        assert "WAL" in text
    ...
    async def test_edit_memory(self):
        ...
        node_id = await _store_and_get_id(
            "Use pip install -e for editable installs during development of Cairn packages",
            event_type="decision",
        )
        result = await HANDLERS["cairn_edit_memory"]({
            "memory_id": node_id,
            "new_content": "Use pip install -e . for editable installs during Cairn development (requires pyproject.toml)",
        })
        assert not _is_error(result)
        text = _text(result)
    ...
    async def test_delete_memory(self):
        ...
        result = await HANDLERS["cairn_query"]({"query": "Temporary test memory deleted verification"})
        text = _text(result)
    ...
    async def test_store_empty_content_rejected(self):
        ...
        result = await HANDLERS["cairn_store"]({"content": "", "event_type": "memory"})
        assert _is_error(result)
    ...
    async def test_phrase_search(self):
        ...
        result = await HANDLERS["cairn_phrase_search"]({"phrase": "bge-small"})
        assert not _is_error(result)
        text = _text(result)
...
class TestUATMemoryEvolution:
    ...
    async def test_evolution_only_for_eligible_types(self):
        ...
        await _store_and_get_id(
            "Session summary for testing the evolution eligibility of different memory types in Cairn system",
            event_type="session_summary",
        )
        result = await HANDLERS["cairn_store"]({
            "content": "Session summary for testing the evolution eligibility of different memory types in Cairn. Additional context about the session outcome and next steps",
            "event_type": "session_summary",
        })
        text = _text(result)
    ...
    async def test_error_pattern_dedup(self):
        ...
        await _store_and_get_id(
            "ImportError: cannot import name 'CairnMemory' from 'cairn.bridge' at /Users/test/proj/cairn/bridge.py",
            event_type="error_pattern",
        )
        result = await HANDLERS["cairn_store"]({
            "content": "ImportError: cannot import name 'CairnMemory' from 'cairn.bridge' at /Users/different/proj/cairn/bridge.py",
            "event_type": "error_pattern",
        })
        text = _text(result)
        assert "Deduped" in text or "reused" in text.lower()
    ...
    async def test_type_stats_after_stores(self):
        ...
        await _store_and_get_id("Decision to use SQLite instead of PostgreSQL for the Cairn memory backend", event_type="decision")
        ...
        result = await HANDLERS["cairn_type_stats"]({})
        assert not _is_error(result)
        text = _text(result)
...
class TestUATKnowledgeGraph:
    ...
    async def test_query_returns_results_for_related_content(self):
        ...
        await _store_and_get_id(
            "Cairn uses bge-small-en-v1.5 ONNX model for embedding generation in the vector search pipeline",
            event_type="lesson_learned",
        )
        await _store_and_get_id(
        ...
        result = await HANDLERS["cairn_query"]({"query": "embedding model vector search"})
        text = _text(result)
    ...
    async def test_traverse_nonexistent_memory(self):
        ...
        result = await HANDLERS["cairn_traverse"]({"memory_id": "nonexistent-id-99999"})
        assert not _is_error(result)
        text = _text(result)
    ...
    async def test_similar_nonexistent_memory(self):
        ...
        result = await HANDLERS["cairn_similar"]({"memory_id": "nonexistent-id-99999"})
        assert not _is_error(result)
        text = _text(result)
        assert "not found" in text.lower()
    ...
    async def test_timeline_shows_recent(self):
        ...
        await _store_and_get_id(
            "Timeline test memory about configuring the Cairn MCP server with stdio transport",
            event_type="decision",
        )
        result = await HANDLERS["cairn_timeline"]({"days": 1})
        assert not _is_error(result)
        text = _text(result)
...
class TestUATSessionScoping:
    ...
    async def test_store_with_session_id(self):
        ...
        result = await HANDLERS["cairn_store"]({
            "content": "Session-scoped memory for testing isolation between different agent sessions in Cairn",
            "event_type": "lesson_learned",
    ...
    async def test_query_scoped_by_session(self):
        ...
        await HANDLERS["cairn_store"]({
            "content": "Session AAA memory about configuring the pytest-asyncio fixture mode for Cairn tests",
            "event_type": "lesson_learned",
    ...
    async def test_clear_session(self):
        ...
        await HANDLERS["cairn_store"]({
            "content": "Clearable session memory about Cairn hook configuration and management strategies",
            "event_type": "lesson_learned",
            "session_id": "session-to-clear",
        })
        await HANDLERS["cairn_store"]({
            "content": "Persistent session memory about the Cairn bridge singleton pattern implementation details",
            "event_type": "lesson_learned",
            "session_id": "session-to-keep",
        })

        result = await HANDLERS["cairn_clear_session"]({"session_id": "session-to-clear"})
        assert not _is_error(result)
        text = _text(result)
    ...
    async def test_session_stats(self):
        ...
        await HANDLERS["cairn_store"]({
            "content": "Stats test memory for verifying session statistics reporting in the Cairn system",
            "event_type": "lesson_learned",
            "session_id": "stats-session-1",
        })
        result = await HANDLERS["cairn_session_stats"]({})
        assert not _is_error(result)
        text = _text(result)
...
class TestUATPreferenceManagement:
    ...
    async def test_remember_stores_preference(self):
        """UAT: cairn_remember stores a user preference."""
        result = await HANDLERS["cairn_remember"]({
    ...
    async def test_remember_empty_rejected(self):
        """UAT: cairn_remember rejects empty text."""
...
class TestUATConsolidationPipeline:
    ...
    async def test_consolidate_with_data(self):
        ...
        for i in range(5):
            await HANDLERS["cairn_store"]({
                "content": f"Consolidation test lesson number {i} about Cairn memory management and optimization strategies",
                "event_type": "lesson_learned",
            })
        result = await HANDLERS["cairn_consolidate"]({"prune_days": 1, "wait": True})
        assert not _is_error(result)
        text = _text(result)
    ...
    async def test_compact_dry_run(self):
        ...
        result = await HANDLERS["cairn_compact"]({
            "event_type": "lesson_learned",
            "dry_run": True,
            "wait": True,
        })
...
class TestUATWelcomeBriefing:
    ...
    async def test_welcome_with_memories(self):
        ...
        await _store_and_get_id(
            "Key decision: use SQLite-vec for vector similarity search instead of Faiss in Cairn",
            event_type="decision",
        )
        await _store_and_get_id(
    ...
    async def test_welcome_prioritizes_high_value(self):
        ...
        await HANDLERS["cairn_store"]({
            "content": "Session summary: worked on Cairn test infrastructure and hook configuration",
            "event_type": "session_summary",
        })
        await _store_and_get_id(
            "Critical decision: Cairn uses ONNX CPU-only inference to avoid the CoreML memory leak issue",
            event_type="decision",
        )
        result = await HANDLERS["cairn_welcome"]({})
        text = _text(result)
    ...
    async def test_welcome_with_session_id(self):
        ...
        result = await HANDLERS["cairn_welcome"]({"session_id": "test-session-welcome"})
        assert not _is_error(result)
...
class TestUATExportImport:
    ...
    @pytest.mark.asyncio
    async def test_export_creates_file(self, tmp_cairn_dir):
        """UAT: Export creates a backup file."""
        await _store_and_get_id(
            "Export test memory about Cairn backup and restore procedures for data safety",
            event_type="decision",
        )
    ...
    @pytest.mark.asyncio
    async def test_export_import_roundtrip(self, tmp_cairn_dir):
        """UAT: Export then import restores memories."""
        await _store_and_get_id(
            "Roundtrip export import test about Cairn memory persistence and data integrity",
            event_type="lesson_learned",
        )
    ...
    async def test_backup_requires_filepath(self):
        ...
        result = await HANDLERS["cairn_backup"]({"mode": "export", "filepath": ""})
        assert _is_error(result)
...
class TestUATFeedbackLoop:
    ...
    async def test_feedback_outdated(self):
        ...
        node_id = await _store_and_get_id(
            "Outdated lesson about configuring the old JSONL store backend for Cairn memory persistence",
            event_type="lesson_learned",
        )
        result = await HANDLERS["cairn_feedback"]({
            "memory_id": node_id,
    ...
    async def test_feedback_invalid_rating(self):
        ...
        node_id = await _store_and_get_id(
            "Memory for testing invalid feedback ratings in the Cairn feedback system handler",
            event_type="lesson_learned",
        )
        result = await HANDLERS["cairn_feedback"]({
            "memory_id": node_id,
    ...
    async def test_feedback_cumulative(self):
        ...
        node_id = await _store_and_get_id(
            "Memory for testing cumulative feedback signals across multiple ratings in Cairn system",
            event_type="lesson_learned",
        )
        # First feedback
        result1 = await HANDLERS["cairn_feedback"]({
            "memory_id": node_id,
            "rating": "helpful",
        })
        ...
        # Second feedback
        result2 = await HANDLERS["cairn_feedback"]({
            "memory_id": node_id,
            "rating": "helpful",
        })
...
class TestUATContentSafety:
    ...
    async def test_blocklist_work_breadcrumb_rejected(self):
        ...
        result = await HANDLERS["cairn_store"]({
            "content": "[WORK BREADCRUMB] Agent started working on file cairn/bridge.py",
            "event_type": "memory",
        })
        text = _text(result)
        assert "Blocked" in text or "noise" in text.lower()
