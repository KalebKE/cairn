"""
Comprehensive tests for the 13 core MCP tool handlers in cairn.server.handlers.

Covers happy paths and key edge cases for:
  1. cairn_store          8. cairn_memory (composite)
  2. cairn_query           9. cairn_profile (skip -- covered in test_handler_coverage.py)
  3. cairn_welcome        10. cairn_remind (composite)
  4. cairn_protocol       11. cairn_maintain (composite)
  5. (removed)            12. cairn_stats (composite)
  6. cairn_checkpoint     13. cairn_reflect (composite)
  7. cairn_resume_task
...
def _fresh_state(tmp_cairn_dir):
...
def _store(content="test memory", event_type="memory", **kwargs) -> tuple[dict, str | None]:
    """Store a memory via handle_cairn_store, return (result, mem_id)."""
    from cairn.server.handlers import handle_cairn_store
    args = {"content": content, "event_type": event_type, **kwargs}
    result = run_async(handle_cairn_store(args))
    mem_id = _extract_mem_id(_text(result)) if not _is_error(result) else None
    return result, mem_id


# ===========================================================================
# 1. cairn_store (handle_cairn_store)
# ===========================================================================
...
class TestCairnStore:
    ...
    def test_store_basic_memory(self):
        from cairn.server.handlers import handle_cairn_store
        result = run_async(handle_cairn_store({"content": "Basic test memory"}))
    ...
    def test_store_with_event_type(self):
        from cairn.server.handlers import handle_cairn_store
        result = run_async(handle_cairn_store({
    ...
    def test_store_empty_content_error(self):
        from cairn.server.handlers import handle_cairn_store
        result = run_async(handle_cairn_store({"content": ""}))
    ...
    def test_store_text_alias(self):
        """cairn_remember backward compat: 'text' works as alias for 'content'."""
        from cairn.server.handlers import handle_cairn_store
        result = run_async(handle_cairn_store({"text": "Remember this via text alias"}))
        assert not _is_error(result)
        assert "mem-" in _text(result)
    ...
    def test_store_decision_broadcasts(self):
        ...
        r1 = run_async(handle_cairn_store({
        ...
        r2 = run_async(handle_cairn_store({
...
# ===========================================================================
# 2. cairn_query (handle_cairn_query)
# ===========================================================================
...
class TestCairnQuery:
    ...
    def test_query_semantic_returns_results(self):
        ...
        result = run_async(handle_cairn_query({"query": "database migration alembic"}))
        assert not _is_error(result)
        text = _text(result)
    ...
    def test_query_never_degrades_and_never_upsells(self, monkeypatch):
        ...
        result = run_async(handle_cairn_query({"query": "database migration"}))
...
# ===========================================================================
# 3. cairn_welcome (handle_cairn_welcome)
# ===========================================================================


class TestCairnWelcome:

    def test_welcome_basic(self):
        from cairn.server.handlers import handle_cairn_welcome
        result = run_async(handle_cairn_welcome({}))
        assert not _is_error(result)
        text = _text(result)
    ...
    def test_welcome_with_session(self):
        from cairn.server.handlers import handle_cairn_welcome
        result = run_async(handle_cairn_welcome({"session_id": "test-welcome-session"}))
        assert not _is_error(result)
        assert len(_text(result)) > 0

    def test_welcome_with_project(self):
        from cairn.server.handlers import handle_cairn_welcome
        result = run_async(handle_cairn_welcome({"project": "/tmp/test-project"}))
        assert not _is_error(result)
...
# ===========================================================================
# 4. cairn_protocol (handle_cairn_protocol)
# ===========================================================================


# TestCairnProtocol removed — protocol module is pro-only


# ===========================================================================
# 5. cairn_lessons — REMOVED (auto-surfaced via hooks, 0 calls ever)
# ===========================================================================


# ===========================================================================
# 6. cairn_checkpoint (handle_cairn_checkpoint)
# ===========================================================================
...
class TestCairnCheckpoint:
    ...
    def test_checkpoint_basic(self):
        from cairn.server.handlers import handle_cairn_checkpoint
        result = run_async(handle_cairn_checkpoint({
    ...
    def test_checkpoint_numbering_increments(self):
        ...
        from cairn.server.handlers import handle_cairn_checkpoint
        r1 = run_async(handle_cairn_checkpoint({
        ...
        r2 = run_async(handle_cairn_checkpoint({
            "task_title": "Numbering test task",
...
# ===========================================================================
# 7. cairn_resume_task (handle_cairn_resume_task)
# ===========================================================================
...
class TestCairnResumeTask:
    ...
    def test_resume_finds_checkpoint(self):
        ...
        from cairn.server.handlers import handle_cairn_checkpoint, handle_cairn_resume_task
        run_async(handle_cairn_checkpoint({
            "task_title": "Resume test task",
            "progress": "Halfway through implementation",
            "next_steps": "Finish the REST endpoints",
        }))
        result = run_async(handle_cairn_resume_task({
            "task_title": "Resume test task",
        }))
        assert not _is_error(result)
        text = _text(result)
    ...
    def test_resume_no_checkpoints(self):
        from cairn.server.handlers import handle_cairn_resume_task
        result = run_async(handle_cairn_resume_task({
...
# ===========================================================================
# 8. cairn_memory composite (handle_cairn_memory)
# ===========================================================================


class TestCairnMemoryDelete:

    def test_memory_delete(self):
        from cairn.server.handlers import handle_cairn_memory
        _, mem_id = _store("Memory to be deleted")
        assert mem_id is not None
        result = run_async(handle_cairn_memory({
            "action": "delete",
            "memory_id": mem_id,
        }))
        assert not _is_error(result)
...
class TestCairnMemorySimilar:
    ...
    def test_memory_similar_missing_id(self):
        from cairn.server.handlers import handle_cairn_memory
        result = run_async(handle_cairn_memory({"action": "similar"}))
        assert _is_error(result)
...
class TestCairnMemoryFlagged:
    ...
    def test_memory_flagged_empty(self):
        from cairn.server.handlers import handle_cairn_memory
        result = run_async(handle_cairn_memory({"action": "flagged"}))
        assert not _is_error(result)
        text = _text(result)
...
class TestCairnMemoryUnknownAction:

    def test_memory_unknown_action(self):
        from cairn.server.handlers import handle_cairn_memory
        result = run_async(handle_cairn_memory({"action": "teleport"}))
        assert _is_error(result)
        assert "unknown" in _text(result).lower()


# ===========================================================================
# 10. cairn_remind composite (handle_cairn_remind_composite)
# ===========================================================================


class TestCairnRemind:

    def test_remind_set(self):
        from cairn.server.handlers import handle_cairn_remind_composite
        result = run_async(handle_cairn_remind_composite({
            "action": "set",
    ...
    def test_remind_list(self):
        ...
        run_async(handle_cairn_remind_composite({
            "action": "set",
            "text": "Listed reminder",
            "duration": "2h",
        }))
        result = run_async(handle_cairn_remind_composite({"action": "list"}))
        assert not _is_error(result)
        text = _text(result)
    ...
    def test_remind_dismiss(self):
        ...
        set_result = run_async(handle_cairn_remind_composite({
            "action": "set",
    ...
    def test_remind_dismiss_missing_id(self):
        from cairn.server.handlers import handle_cairn_remind_composite
        result = run_async(handle_cairn_remind_composite({"action": "dismiss"}))
        assert _is_error(result)

    def test_remind_unknown_action(self):
        from cairn.server.handlers import handle_cairn_remind_composite
        result = run_async(handle_cairn_remind_composite({"action": "snooze"}))
        assert _is_error(result)


# ===========================================================================
# 11. cairn_maintain composite (handle_cairn_maintain)
# ===========================================================================


class TestCairnMaintain:

    def test_maintain_health(self):
        from cairn.server.handlers import handle_cairn_maintain
        result = run_async(handle_cairn_maintain({"action": "health"}))
        assert not _is_error(result)
        text = _text(result)
    ...
    def test_maintain_consolidate(self):
        ...
        result = run_async(handle_cairn_maintain({"action": "consolidate", "wait": True}))
        assert not _is_error(result)

    def test_maintain_compact(self):
        from cairn.server.handlers import handle_cairn_maintain
        result = run_async(handle_cairn_maintain({"action": "compact", "wait": True}))
        assert not _is_error(result)
    ...
    def test_maintain_backup_restore(self):
        ...
        try:
            export_result = run_async(handle_cairn_maintain({
                "action": "backup",
            ...
            # Now restore
            restore_result = run_async(handle_cairn_maintain({
                "action": "restore",
                "filepath": export_path,
                "wait": True,
            }))
    ...
    def test_maintain_backup_missing_filepath(self):
        from cairn.server.handlers import handle_cairn_maintain
        result = run_async(handle_cairn_maintain({"action": "backup"}))
        assert _is_error(result)

    def test_maintain_backup_unsafe_path(self):
        """Paths outside ~/.cairn should be rejected."""
        from cairn.server.handlers import handle_cairn_maintain
        result = run_async(handle_cairn_maintain({
            "action": "backup",
    ...
    def test_maintain_unknown_action(self):
        from cairn.server.handlers import handle_cairn_maintain
        result = run_async(handle_cairn_maintain({"action": "nuke"}))
        assert _is_error(result)
        assert "unknown" in _text(result).lower()


# ===========================================================================
# 12. cairn_stats composite (handle_cairn_stats)
# ===========================================================================


class TestCairnStats:
    ...
    def test_stats_types(self):
        ...
        result = run_async(handle_cairn_stats({"action": "types"}))
        assert not _is_error(result)
        text = _text(result)
    ...
    def test_stats_sessions(self):
        ...
        result = run_async(handle_cairn_stats({"action": "sessions"}))
        assert not _is_error(result)
    ...
    def test_stats_digest(self):
        ...
        result = run_async(handle_cairn_stats({"action": "digest"}))
        assert not _is_error(result)
        text = _text(result)
    ...
    def test_stats_dedup(self):
        from cairn.server.handlers import handle_cairn_stats
        result = run_async(handle_cairn_stats({"action": "dedup"}))
        assert not _is_error(result)
        text = _text(result)
    ...
    def test_stats_forgetting_log(self):
        from cairn.server.handlers import handle_cairn_stats
        result = run_async(handle_cairn_stats({"action": "forgetting_log"}))
        assert not _is_error(result)

    def test_stats_unknown_action(self):
        from cairn.server.handlers import handle_cairn_stats
        result = run_async(handle_cairn_stats({"action": "quantum"}))
        assert _is_error(result)

    def test_stats_diagnostic(self):
        from cairn.server.handlers import handle_cairn_stats
        _store("Diagnostic test memory", event_type="decision")
        result = run_async(handle_cairn_stats({"action": "diagnostic"}))
        assert not _is_error(result)
        text = _text(result)
        import json
    ...
    def test_stats_diagnostic_custom_days(self):
        from cairn.server.handlers import handle_cairn_stats
        result = run_async(handle_cairn_stats({"action": "diagnostic", "days": 7}))
        assert not _is_error(result)
        text = _text(result)
        import json
        report = json.loads(text)
...
# ===========================================================================
# 13. cairn_reflect composite (handle_cairn_reflect)
# ===========================================================================
...
class TestCairnReflect:
    ...
    def test_reflect_contradictions(self):
        from cairn.server.handlers import handle_cairn_reflect
        ...
        _store("The API uses GraphQL instead of REST for all communication",
               event_type="decision")
        result = run_async(handle_cairn_reflect({
    ...
    def test_reflect_contradictions_missing_topic(self):
        from cairn.server.handlers import handle_cairn_reflect
        result = run_async(handle_cairn_reflect({"action": "contradictions"}))
        assert _is_error(result)
    ...
    def test_reflect_evolution_missing_topic(self):
        from cairn.server.handlers import handle_cairn_reflect
        result = run_async(handle_cairn_reflect({"action": "evolution"}))
        assert _is_error(result)

    def test_reflect_stale(self):
        from cairn.server.handlers import handle_cairn_reflect
        # Stale audit works on old memories; with fresh store it should return "no stale"
        result = run_async(handle_cairn_reflect({"action": "stale"}))
        assert not _is_error(result)
        text = _text(result)
    ...
    def test_reflect_unknown_action(self):
        from cairn.server.handlers import handle_cairn_reflect
        result = run_async(handle_cairn_reflect({"action": "meditate"}))
        assert _is_error(result)
        assert "unknown" in _text(result).lower()
...
class TestValidateMemoryWrite:
    ...
    def test_normalizes_string_metadata(self):
        from cairn.server.handlers import _validate_memory_write
...
class TestClampInt:
    ...
    def test_clamp_normal(self):
        from cairn.server.handlers import _clamp_int
