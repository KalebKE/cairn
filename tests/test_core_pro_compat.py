...
# Pro features live under the ``cairn_platform.*`` namespace, never ``cairn.*``.
# Core source that imports them from ``cairn.<module>`` is the issue #64 bug:
...
_CORE_SRC = Path(__file__).resolve().parent.parent / "src" / "cairn"


def test_bridge_submodule_aliases_support_pro_imports():
    core = importlib.import_module("cairn.bridge._core")
    ingest = importlib.import_module("cairn.bridge._ingest")
    query = importlib.import_module("cairn.bridge._query")
...
def test_pid_registry_imports_and_formats_diagnostics(tmp_path, monkeypatch):
    monkeypatch.setattr("cairn.server.pid_registry._REGISTRY_DIR", tmp_path)

    registry = importlib.import_module("cairn.server.pid_registry")
    entry = registry.register_pid(transport="stdio")

    assert entry["pid"] > 0
    assert registry.list_registered_pids()
    assert "Registered Cairn processes" in registry.format_lock_diagnostic()

    registry.unregister_pid()
    assert registry.list_registered_pids() == []


def test_core_never_imports_pro_features_from_cairn_namespace():
    """Core/Pro boundary regression: Pro code is never under ``cairn.*``.

    Scans every Core source file for ``from cairn.<pro> import ...`` or
    ``import cairn.<pro>`` where ``<pro>`` is a Pro-only top-level module. Those
    ...
    for path in _CORE_SRC.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom) and node.module:
                ...
                if head == "cairn" and tail.split(".")[0] in _PRO_ONLY_TOP_LEVEL:
    ...
    assert not offenders, (
        "Core imports Pro features from the non-existent cairn.* namespace "
